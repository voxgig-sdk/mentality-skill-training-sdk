...
// MentalitySkillTraining SDK TrainingProgram entity
...
class TrainingProgramEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new TrainingProgramEntity($this->_client, $opts);
    }

    /**
     * @param TrainingProgram|array $args TrainingProgram data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = MentalitySkillTrainingHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return TrainingProgram|array The current TrainingProgram data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of TrainingProgram fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = MentalitySkillTrainingHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of TrainingProgram fields).
     */
    ...
    /**
     * List TrainingProgram items matching the given filter.
     *
     * @param TrainingProgramListMatch|array|null $reqmatch Match filter (any subset
     *   of TrainingProgram fields) as an assoc-array; TrainingProgramListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return TrainingProgram[]|array A list of TrainingProgram items as assoc-arrays at
