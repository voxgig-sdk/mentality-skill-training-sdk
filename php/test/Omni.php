...
//    only - a typed MentalitySkillTrainingContext there would make every ctx
...
final class MentalitySkillTrainingOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new MentalitySkillTrainingContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new MentalitySkillTrainingSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new MentalitySkillTrainingResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new MentalitySkillTrainingError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new MentalitySkillTrainingResponse($clean['response']);
    ...
    public static function err_from_map($m): ?MentalitySkillTrainingError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new MentalitySkillTrainingError(is_string($code) ? $code : '', $msg);
    }
