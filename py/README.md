# MentalitySkillTraining Python SDK

The Python SDK for the MentalitySkillTraining API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Exercis(None).list(None, None)
if err:
    raise Exception(err)
...
result, err = client.MentalitySkillTraining(None).load(
    {"id": "test01"}, None
)
...
```python
client = MentalitySkillTrainingSDK.test(testopts, sdkopts)
```
...
```
py/
├── mentalityskilltraining_sdk.py         -- Main SDK module
...
The main module (`mentalityskilltraining_sdk`) exports the SDK class.
