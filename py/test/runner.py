# MentalitySkillTraining SDK test runner
...
class MentalitySkillTrainingTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            MentalitySkillTrainingTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = MentalitySkillTrainingTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = MentalitySkillTrainingTestRunner.getenv("MENTALITY_SKILL_TRAINING_TEST_LIVE")
        override = MentalitySkillTrainingTestRunner.getenv("MENTALITY_SKILL_TRAINING_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = MentalitySkillTrainingTestRunner.getenv(key)
        ...
        explain = MentalitySkillTrainingTestRunner.getenv("MENTALITY_SKILL_TRAINING_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["MENTALITY_SKILL_TRAINING_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if MentalitySkillTrainingTestRunner._test_control is not None:
            return MentalitySkillTrainingTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                MentalitySkillTrainingTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            MentalitySkillTrainingTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = MentalitySkillTrainingTestRunner.load_test_control()
...
def load_env_local():
    MentalitySkillTrainingTestRunner.load_env_local()


def env_override(m):
    return MentalitySkillTrainingTestRunner.env_override(m)


def entity_data(v):
    return MentalitySkillTrainingTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return MentalitySkillTrainingTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return MentalitySkillTrainingTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return MentalitySkillTrainingTestRunner.load_test_control()


def live_client_options():
    return MentalitySkillTrainingTestRunner.live_client_options()


def live_delay_ms():
    return MentalitySkillTrainingTestRunner.live_delay_ms()
