...
class TestTrainingProgramEntity:
    ...
    def test_should_stream(self):
        ...
        seen = list(base.TrainingProgram(None).stream("list", None, None))
        assert len(seen) == 3

        # Inbound: streaming active -> yields each item from the feature.
        from mentalityskilltraining_sdk.config import shared_config
        cfg = shared_config()
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            sdk = MentalitySkillTrainingSDK.test(
                seed, {"feature": {"streaming": {"active": True}}})
            got = []
            for item in sdk.TrainingProgram(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set MENTALITY_SKILL_TRAINING_TEST_TRAINING_PROGRAM_ENTID JSON to run live")
        client = setup["client"]
...
def _training_program_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("MENTALITY_SKILL_TRAINING_TEST_TRAINING_PROGRAM_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("MENTALITY_SKILL_TRAINING_TEST_LIVE") == "TRUE":
    ...
    _live = env.get("MENTALITY_SKILL_TRAINING_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("MENTALITY_SKILL_TRAINING_TEST_EXPLAIN") == "TRUE",
        "live": _live,
