...
class TestTrainingProgramDirect:

    def test_should_direct_list_training_program(self):
        setup = _training_program_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-training_program", "live" if setup["live"] else "unit")
        if _skip:
...
def _training_program_direct_setup(mockres):
