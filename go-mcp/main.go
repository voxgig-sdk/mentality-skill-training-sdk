// Command MentalitySkillTraining-mcp is an MCP server exposing the MentalitySkillTraining
// SDK as MCP tools.
//
// Two tools are registered:
//
//	MentalitySkillTraining_list  — list records of an entity
//	MentalitySkillTraining_load  — load a single record of an entity
//
...
func main() {
	...
	// Configure from the environment: MENTALITY_SKILL_TRAINING_APIKEY carries the API key and
	// MENTALITY_SKILL_TRAINING_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("MENTALITY_SKILL_TRAINING_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("MENTALITY_SKILL_TRAINING_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewMentalitySkillTrainingSDK(opts)
