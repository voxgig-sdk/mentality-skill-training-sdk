# mentality-skill-training-mcp

[MCP](https://modelcontextprotocol.io) server exposing the MentalitySkillTraining SDK as
two agent tools — `mentality-skill-training_list` and `mentality-skill-training_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/mentality-skill-training-mcp)
make build
...
claude mcp add --scope user mentality-skill-training \
  -- /absolute/path/to/mentality-skill-training-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./mentality-skill-training-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// mentality-skill-training_list: first page of records
{ "entity": "exercis" }
{ "entity": "exercis", "query": { } }

// mentality-skill-training_load: one record by id
{ "entity": "exercis", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/mentality-skill-training-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user mentality-skill-training \
     -- "$PWD"/dist/*/mentality-skill-training-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `mentality-skill-training_list` and `mentality-skill-training_load` tools now appear
   in new sessions. Ask the agent to *"list exercis using mentality-skill-training"*
   and it calls `mentality-skill-training_list` with `{"entity":"exercis"}`.
...
### Call the `mentality-skill-training_list` tool
...
### Call the `mentality-skill-training_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `mentality-skill-training_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `mentality-skill-training_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `MENTALITY_SKILL_TRAINING_APIKEY` | API key sent with every request. |
| `MENTALITY_SKILL_TRAINING_BASE` | Optional override of the API base URL. |
...
```sh
./mentality-skill-training-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"mentality-skill-training_load","arguments":{"entity":"exercis","query":{"id":1}}}}'
```
