...
class TrainingProgramEntity
  ...
  def make
    opts = @_entopts.dup
    TrainingProgramEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = MentalitySkillTrainingHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [TrainingProgram, Hash] the current TrainingProgram data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = MentalitySkillTrainingHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of TrainingProgram fields)
  def match_get
  ...
  #
  # @param reqmatch [TrainingProgramListMatch, Hash, nil] match filter (any subset of
  #   TrainingProgram fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<TrainingProgram>, Array] the matching TrainingProgram items; raises MentalitySkillTrainingError on failure
