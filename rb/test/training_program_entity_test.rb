...
class TrainingProgramEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.TrainingProgram(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.TrainingProgram(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set MENTALITY_SKILL_TRAINING_TEST_TRAINING_PROGRAM_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    training_program_ref01_list_result = training_program_ref01_ent.list(training_program_ref01_match, nil)
...
def training_program_basic_setup(extra)
  ...
  entid_env_raw = ENV["MENTALITY_SKILL_TRAINING_TEST_TRAINING_PROGRAM_ENTID"]
