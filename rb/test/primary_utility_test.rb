...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = MentalitySkillTrainingOmni.make_runner(TEST_JSON_FILE, MentalitySkillTrainingSDK.test(nil, nil))
  ...
  def err_from_map(m)
    ...
    MentalitySkillTrainingError.new(m["code"] || "", msg)
  end
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : MentalitySkillTrainingOmni.errify(err)
    end
  ...
  def test_operator_basic
    subject = lambda do |vin|
      vin = {} unless vin.is_a?(Hash)
      op = MentalitySkillTrainingOperation.new(vin)
      {
        "entity" => op.entity,
        "name" => op.name,
        "input" => op.input,
