# MentalitySkillTraining Lua SDK

The Lua SDK for the MentalitySkillTraining API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Exercis(nil):list(nil, nil)
...
local result, err = client:MentalitySkillTraining(nil):load(
  { id = "test01" }, nil
)
...
```
lua/
├── mentality-skill-training_sdk.lua    -- Main SDK module
...
The main module (`mentality-skill-training_sdk`) exports the SDK constructor
