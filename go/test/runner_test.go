...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("MENTALITYSKILLTRAINING_TEST_LIVE") == "TRUE" ||
		os.Getenv("MENTALITYSKILLTRAINING_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("MENTALITYSKILLTRAINING_TEST_EXPLAIN"); explain != "" {
