...
import (
	...
	omni "github.com/voxgig-sdk/mentality-skill-training-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.MentalitySkillTrainingError); is {
				out := map[string]any{
					"name":    "MentalitySkillTrainingError",
...
func omniCtx(arg any, client *sdk.MentalitySkillTrainingSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.MentalitySkillTrainingSDK); is {
			client = live
