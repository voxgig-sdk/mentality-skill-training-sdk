...
func TestTrainingProgramEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set MENTALITY_SKILL_TRAINING_TEST_TRAINING_PROGRAM_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		trainingProgramRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.training_program")))
...
func training_programBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("MENTALITY_SKILL_TRAINING_TEST_TRAINING_PROGRAM_ENTID")
	...
	idmapResolved := core.ToMapAny(env["MENTALITY_SKILL_TRAINING_TEST_TRAINING_PROGRAM_ENTID"])
