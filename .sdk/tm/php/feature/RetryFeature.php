...
// MentalitySkillTraining SDK retry feature
...
class MentalitySkillTrainingRetryFeature extends MentalitySkillTrainingBaseFeature
{
    ...
    public function init(MentalitySkillTrainingContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (MentalitySkillTrainingContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(MentalitySkillTrainingContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(MentalitySkillTrainingContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
