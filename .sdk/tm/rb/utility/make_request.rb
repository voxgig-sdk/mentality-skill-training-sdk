...
module MentalitySkillTrainingUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = MentalitySkillTrainingResponse.new({})
    ...
    if fetch_err
      ...
      response = MentalitySkillTrainingResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = MentalitySkillTrainingResponse.new(fetched)
    else
