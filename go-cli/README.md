...
AQL-driven command-line client **and** interactive REPL for the MentalitySkillTraining
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/mentality-skill-training-cli)
make build

# 2. See usage (words, entities, env vars)
./mentality-skill-training-cli --help
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/mentality-skill-training-cli
   ```
...
Run with no arguments to open a REPL (prompt `mentality-skill-training>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an AQL atom (e.g. `exercis`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/mentality-skill-training-cli`. |
...
(`./mentality-skill-training-cli <expr>`) and interactively (the REPL), and expressions compose the
