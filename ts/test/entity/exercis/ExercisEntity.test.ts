...
describe('ExercisEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list']) {
      if (maybeSkipControl(t, 'entityOp', 'exercis.' + op, live)) return
    }
    ...
    const exercis_ref01_list = await exercis_ref01_ent.list(exercis_ref01_match)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['MENTALITY_SKILL_TRAINING_TEST_EXERCIS_ENTID']
