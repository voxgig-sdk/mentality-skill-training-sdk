...
//     (\`new MentalitySkillTrainingSDK(...)\` / \`MentalitySkillTrainingSDK.test(...)\` become
//     \`MentalitySkillTrainingSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'MentalitySkillTrainingSDK'
...
// rewrite real client construction (\`new MentalitySkillTrainingSDK(...)\` and any
// \`MentalitySkillTrainingSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+MentalitySkillTrainingSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/MentalitySkillTrainingSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
